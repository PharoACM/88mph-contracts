...
contract('TransferrableVesting', accounts => {
  ...
  beforeEach(async () => {
    ...
    vesting = await TransferrableVesting.new(token.address)
  })
  ...
  it('should vest to new address after NFT transfer', async () => {
    // acc0 vest 10 tokens to acc1 for 7 days
    const vestAmount = 10 * TOKEN_PRECISION
    const vestPeriod = 7 * DAY
    await token.approve(vesting.address, num2str(vestAmount), { from: acc0 })
    ...
    // acc1 transfer vest NFT to acc2
    await vesting.transferFrom(acc1, acc2, 0, { from: acc1 })

    // acc2 withdraws vested amount
    const acc2BeforeBalance = BigNumber(await token.balanceOf(acc2))
    await vesting.withdrawVested(acc2, 0, { from: acc0 })

    // verify amount
    const receivedAmount = BigNumber(await token.balanceOf(acc2)).minus(acc2BeforeBalance)
    const expectedReceiveAmount = BigNumber(vestAmount)
    assert(receivedAmount.eq(expectedReceiveAmount), 'fully vested amount incorrect')
  })

  it('should not vest to old address after NFT transfer', async () => {
    // acc0 vest 10 tokens to acc1 for 7 days
    const vestAmount = 10 * TOKEN_PRECISION
    const vestPeriod = 7 * DAY
    await token.approve(vesting.address, num2str(vestAmount), { from: acc0 })
    ...
    // verify amount
    const receivedAmount = BigNumber(await token.balanceOf(acc1)).minus(acc1BeforeBalance)
    const expectedReceiveAmount = 0
    assert(receivedAmount.eq(expectedReceiveAmount), 'vested non-zero amount to old address')
  })
})
